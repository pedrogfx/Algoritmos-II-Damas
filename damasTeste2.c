...
int menu = 0, i = 0, j = 0;
int fimJogo = 0, movimentoInvalido, linhaDestino = 0, colunaDestino = 0, linhaOrigem = 0, colunaOrigem = 0, movimento = 0, jogador = 1; //variaveis das peças
...
int movimentarPeca(){
	...
	if(jogador == 1){//jogada p/ peça preta
		...
		{	
			if(pecas[matriz[linhaOrigem-1][colunaOrigem-1]] == 0){ //se o movimento for p/ as peças pretas, -1 casa p/ subir
				pecas[matriz[linhaOrigem][colunaOrigem]] = 0; //resetar ela p/ olocar p/ peças pretas
				pecas[matriz[linhaOrigem-1][colunaOrigem-1]] = B; //colocar a peça preta
				movimentoInvalido = 0; //movimento errado zerado
				jogador = 0; //trocar a vez do jogador				
			}else{
				if(pecas[matriz[linhaOrigem-1][colunaOrigem-1]] == A && pecas[matriz[linhaOrigem-2][colunaOrigem == 0]])//se houver uma peça branca no caminho faz o movimento -2
				{
					pecas[matriz[linhaOrigem][colunaOrigem]] = 0; //reset
					pecas[matriz[linhaOrigem-1][colunaOrigem-1]] = 0; //-1 p deixar em branco a peça que passou por cima
					pecas[matriz[linhaOrigem-2][colunaOrigem-2]] = B; //colocar a peça preta
					movimentoInvalido = 0; //movimento errado zerado
					jogador = 0; //trocar a vez do jogador
				}else{
			...
			{
				if(pecas[matriz[linhaOrigem-1][colunaOrigem+1]] == 0){ //caso não tenha peça inimiga na diagonal
				...
				if(movimento == 1 && pecas[matriz[linhaOrigem][colunaOrigem]] == A)//moimento p/ direita
		{
			...
			{ //se o movimento não tiver peça inimiga
				pecas[matriz[linhaOrigem][colunaOrigem]] = 0; //zerar posição inicial dela
				pecas[matriz[linhaOrigem+1][colunaOrigem-1]] == A;
				movimentoInvalido = 0;
				jogador = 1;	
			}else{
				if(pecas[matriz[linhaOrigem+1][colunaOrigem-1]] = B && pecas[matriz[linhaOrigem+2][colunaOrigem-2]] == 0)//se houver uma peça inimiga e a proxima estiver vazia executa o movimento
				{
