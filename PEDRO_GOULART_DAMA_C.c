...
colunaDestino = 0, linhaOrigem = 0, colunaOrigem = 0, movimento = 0, jogador = 0, brancaComida = 0, pretaComida = 0; //variaveis das peças
...
void inicioGame(){
	for(iLinha = 0 ; iLinha < 8; iLinha++)
	{
		...
		for(iLinha = 0 ; iLinha < 8; iLinha++)
		{
			for(iColuna = 0 ; iColuna < 8; iColuna++)
			{
				if (((iLinha % 2 == 0) && (iColuna % 2 == 0)) || ((iLinha % 2 == 1) && (iColuna % 2 == 1)))//teste lógico p/ definir os indices
				{
